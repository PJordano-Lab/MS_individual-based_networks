...
```{r}
net.level.ind <- read.csv(here("metrics/net.level.df.csv")) %>%
  rename(niche.overlap.horn.HL = niche.overlap.HL) %>%
  rename(niche.overlap.horn.LL = niche.overlap.LL)

mod.ind <- read.csv(here("metrics/modularity_estimation_gts.csv")) %>%
...
```{r}
net.level.sp <- read.csv(here("metrics/net.level.df.sp.csv")) %>%
  mutate(net_id = str_sub(net_id, 6, 10)) %>%
  rename(niche.overlap.horn.HL = niche.overlap.HL) %>%
  rename(niche.overlap.horn.LL = niche.overlap.LL)

mod.sp <- read.csv(here("metrics/modularity_estimation_gts_sp.csv")) %>%
  rename(net_id = network) %>%
  mutate(net_id = str_sub(net_id, 6, 10)) %>% 
  dplyr::select(-X)
...
```{r}
igraph.metrics <- read.csv(here("metrics/igraph_metrics.csv")) %>%
...
```{r}
all_nets_char <- readxl::read_xlsx(here("selected_networks.xlsx")) %>%
...
Distribution of some network characteristics
```{r}
#par(mfrow = c(3,2))

ggplot(net.level, aes(x = number.of.species.HL)) + 
  geom_histogram(bins = 50) + 
  labs(x = "No. FRUGIVORES", y = NULL) +
  facet_wrap(~type, scales = "free")

ggplot(net.level, aes(x = number.of.species.LL)) + 
  geom_histogram(bins = 50) + 
  labs(x = "No. PLANTS", y = NULL) +
  facet_wrap(~type, scales = "free")

ggplot(net.level, aes(x = number.of.species.HL + number.of.species.LL,)) + 
  geom_histogram(bins = 50) +  
  labs(x = "No. UNIQUE INTERACTIONS", y = NULL) +
  geom_vline(xintercept = 16, linetype = "dashed", color = "red") +
  facet_wrap(~type, scales = "free")

ggplot(net.level, aes(x = net_size)) + 
  geom_histogram(bins = 50) + 
  labs(x = "NETWORK SIZE (No. CELLS)", y = NULL) +
  geom_vline(xintercept = 50, linetype = "dashed", color = "red") +
  facet_wrap(~type, scales = "free")

ggplot(net.level, aes(x = unique.ints)) + 
  geom_histogram(bins = 50) +  
  labs(x = "No. UNIQUE INTERACTIONS", y = NULL) +
...
```{r}
write_csv(net.level.selection, here("metrics/net.level.selection.csv"))
```
