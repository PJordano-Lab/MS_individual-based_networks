...
Generate 1000 random nets with *Swap* algorithm that keeps constant marginal totals and connectance

```{r}
swap.null.list <- vector("list", 105)

for (i in 1:length(nets)) {
  
  print(i)
  
  nulls <- nullmodel(nets[[i]], N = 1000, method = "swap.web")
  ...
  swap.null.list[[i]] <- nulls
  
  names(swap.null.list)[[i]] <- names(nets)[i]
...
swap.null.list.se <- vector("list", 105)

for(i in 1:length(swap.null.list)) {
  ...
  if (control_needed[i] == "YES" & params$type[i] == "ind") {
    ...
    # Apply to each of the 1000 networks (j)
    net_list <- lapply(swap.null.list[[i]], function(net) {
    ...
    swap.null.list.se[[i]] <- net_list
    names(swap.null.list.se)[[i]] <- names(swap.null.list)[[i]]
  }
  
  else {
    
    swap.null.list.se[[i]] <- swap.null.list[[i]]
    names(swap.null.list.se)[[i]] <- names(swap.null.list)[[i]]
  }
}
...
swap.null.list.std <- lapply(swap.null.list.se, gts_list)
names(swap.null.list.std) <- names(swap.null.list)
```
...
Run umbrella function for each *Swap* null model network:

```{r}
web.metrics.swap <- lapply(1:length(swap.null.list.std), function(i) {
  metrics <- process_network(i, swap.null.list.std)
...
web.metrics.swap.df <- data.table::rbindlist(
  lapply(1:length(web.metrics.swap), function(i) {
  data.table::rbindlist(web.metrics.swap[[i]])
  })
  )

web.metrics.swap.df <- web.metrics.swap.df |>
  relocate(type, .before = everything()) |>
  relocate(net_id, .after = type) |>
  relocate(iter, .after = net_id) |>
  mutate(net_code = paste0(type, "_", net_id)) |>
...
glimpse(web.metrics.swap.df)

saveRDS(web.metrics.swap.df, here("data/net_level_nulls_swap.rds"))
```
