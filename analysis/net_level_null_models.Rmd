...
networklevel(nets[[7]], index = "weighted NODF")

wNODF = function(net, factor){
  a = networklevel(net/factor, index = "weighted NODF")
  return(as.numeric(a))
  }

a = lapply(nulls1, wNODF, factor = 1000)
b = lapply(nulls2, wNODF, factor = 10000)
c = lapply(nulls3, wNODF, factor = 100000)

reshape2::melt(a) %>% summarise(mean(value))
reshape2::melt(b) %>% summarise(mean(value))
reshape2::melt(c) %>% summarise(mean(value))

d = lapply(nulls1, compute_bipartite_metrics) 
d = reshape2::melt(d)
