...
```{r}
elton.bird <- readr::read_delim(here("data/animal_traits/BirdFuncDat.txt"), delim = "\t") %>% 
  dplyr::select(frug_sp_elton = Scientific, family = BLFamilyLatin, bmass.elton = `BodyMass-Value`) %>%
  mutate(class = "bird")

elton.mam <- readr::read_delim(here("data/animal_traits/MamFuncDat.txt"), delim = "\t") %>% 
...
write_csv(frugs, here("data/animal_traits/frugivores_bmass.csv"))
