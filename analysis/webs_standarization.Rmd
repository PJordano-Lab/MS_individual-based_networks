...
```{r}
nets_names_ind <- list.files(path = here("networks/nets_se"), pattern = "_int") # For individual-based networks
nets_names_sp <- list.files(path = here("networks/nets_se"), pattern = "sp_")   # For species-based networks
```
...
for (i in 1:length(nets_names_ind)){
  net_file <- paste0("networks/nets_se/", nets_names_ind[[i]])
...
for (i in 1:length(nets_names_sp)){
  net_file <- paste0("networks/nets_se/", nets_names_sp[[i]])
  net <- read.csv(here(net_file))
  if(ncol(net)<2){
    net <- read.csv(here(net_file), sep = ";")
  } else
